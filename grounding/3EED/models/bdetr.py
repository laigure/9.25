...
class BeaUTyDETR(nn.Module):
    ...
    def forward(self, inputs, return_query_features=False):
        """
        Forward pass.
        Args:
            inputs: dict
        ...
        for i in range(self.num_decoder_layers):
            ...
            # The final decoder state is the language-conditioned feature for
            # each box candidate. Keep it separate from the 64-D contrastive
            # projection, which is intended for matching rather than QA input.
            if return_query_features and i == self.num_decoder_layers - 1:
                end_points["last_query_features"] = query
