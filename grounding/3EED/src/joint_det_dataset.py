...
class Joint3DDataset(Dataset):
        ...
        for frame_info in frame_infos:
            ...
            while True:
                if f"bbox3d_obj_{box_id}" in frame_info:
                    ...
                    if f"bbox2d_obj_{box_id}" in frame_info:
                    ...
                    break
                box_id += 1
            if "target_spans" in frame_info:
                spans = frame_info["target_spans"]
                if len(spans) != len(box_info["bbox3d"]):
                    raise ValueError(f"Target spans and boxes differ: {frame_key}")
                for begin, end in spans:
                    if not (0 <= begin < end <= len(caption)):
                        raise ValueError(f"Invalid target span: {frame_key}, {begin}, {end}")
                all_positive = [torch.tensor(span, dtype=torch.long) for span in spans]
            else:
                all_positive = None
    ...
    def _get_3eed_pcd(self, anno):
        ...
        # Keep stochastic sampling for training. Validation/test must be stable:
        # random resampling previously made the same checkpoint produce
        # different grounding metrics on repeated evaluations.
        if self.split == "train":
            rng = np.random
        else:
            sample_key = f"{self.split}:{anno['scan_id']}".encode("utf-8")
            seed = int.from_bytes(hashlib.sha1(sample_key).digest()[:8], "little")
            rng = np.random.default_rng(seed)
        indices = rng.choice(
            N, TARGET_NUM_POINTS, replace=N < TARGET_NUM_POINTS
        )
    ...
    def _get_waymo_multi_target_boxes(self, anno, xyz):
        ...
        for i in range(gt_bbox.shape[0]):
            ...
            # Keep the target identity. The query-point loss gathers this
            # label and uses it to select seeds near each corresponding GT
            # center; assigning every foreground point to 0 supervises only
            # the first target in a multi-object sample.
    ...
    def getitem_waymo_multi(self, index):
        anno = self.annos[index]
        self.random_utt = False
        if "data/3eed/" not in anno["pcd_path"]:
            anno["pcd_path"] = anno["pcd_path"].replace("data/", "data/3eed/")
        if "data/3eed/" not in anno["image_path"]:
        ...
        # Our reconstructed annotations store points and boxes in each source
        # LiDAR view. Rotate every valid target together with the points to the
        # canonical front view used by the single-object Waymo loader. Keep the
        # behavior opt-in because the unpublished official pkl coordinate frame
        # cannot be verified.
        if anno.get("coordinate_frame") == "source_lidar":
            lidar_id = int(anno["scan_id"].split("_")[-1])
            waymo_views = ["F", "FL", "FR", "SL", "SR"]
            num_obj = int(box_label_mask.sum())
            xyz, target_boxes = transform_to_front_view(
                point_cloud[:, :3], gt_bboxes[:num_obj], waymo_views[lidar_id]
            )
            point_cloud[:, :3] = xyz
        ...
        ret_dict = {
            ...
            "object_ids_json": anno["object_ids_json"],
            "coordinate_frame": anno.get("coordinate_frame", "unknown"),
