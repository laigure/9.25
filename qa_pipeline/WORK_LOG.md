...
9. epoch 10 验证：PerTarget Acc@0.25/0.5 为 **74.83%/46.18%**，JointAll-bbf Acc@0.25/0.5 为 **55.71%/26.48%**；N1/N2/N3/N4/N5 的 Joint@0.25 为 **77.20%/60.16%/37.74%/8.70%/0%**。相对 baseline，N2 和 N4 改善，但 N3 下降，整体 Joint@0.25 仅增加 0.22 个百分点。epoch 15 已于 17:31 保存 `ckpt_epoch_15.pth`，验证正在运行。

## 73. QA 目标数平衡、主语轮换与三类问题（2026-09-23）

1. 用户指出三目标 QA 少于二目标，并要求在同一场景中轮换主语、增加不同问法，同时把问题分成距离、方位和避障三类。数据版本升级为 `scene_reasoning_qa_v3_balanced_categories`，公开行新增 `question_category` 和 `target_count`；输入仍只有通用问题文字与有序 288 维 Grounding token。
2. 方位类对场景内所有有向对象对生成问题，并让 A/B/C/D/E 分别作为主语；N≥3 还生成“一个主语相对其余所有对象”的多参照一句话问题。保留物理交换 A/B 的一致性控制。
3. 距离类包含三组任务：每个对象距 ego 是否在 10 米内、所有对象相对 ego 的完整距离名次、以每个对象为参照寻找最近/最远对象。三目标场景使用三套语义等价问法，二目标使用一套，增加的是问题表达与主语/参照组合，没有复制坐标到公开输入。
4. 避障类保留“最近前方障碍物 + 最安全局部动作”，并增加固定左转、直行、右转候选轨迹是否安全的三条判断题。候选安全标签由全部前方 GT box 对候选局部轨迹的最小 clearance 生成；这是固定前向目标下的局部避障，不是带地图的完整路线规划。
5. 远程生成 33,846 条：train 16,356、val 17,490；距离 15,588、方位 12,378、避障 5,880。按 QA 行计 N2/N3/N4/N5 为 15,302/15,576/2,208/760，train 的 N2/N3 为 7,311/8,424，已消除原 QA 行数量上的二/三目标失衡。
6. 同时保留真实场景数量用于防止误读：1,753 个唯一多目标场景；train 的 N2/N3/N4 场景仍只有 709/125/9，val 为 773/106/23/8。问法扩增能平衡监督次数，不能创造新的三/四目标场景多样性。
7. 全量审计：公开 caption 命中 0、公开 description/category/box/center 字段 0、坐标格式命中 0、非规划问题直接暴露 left/right/front/behind 为 0、完整模型输入重复 0；33,846 条答案全部为一句话。
...
## 75. v3 LoRA 三轮训练完成、全量生成中（2026-09-23）

1. LoRA 已完成 3 epoch，每轮训练耗时 1700.8/1718.2/1712.0 秒；末轮 step 6120 loss 0.0054，训练期间未发现 OOM、NaN、CUDA error 或 Traceback。
2. val loss 为 epoch 1/2/3 = 0.7691/0.9208/0.8225，后两轮没有超过第一轮；`best.pt` 正确保留 epoch 1，包含 Projector 与 LoRA 权重，文件 45,495,397 bytes；`last.pt` 也已保存但不作为正式评测权重。
3. 22:09 检查时正在用 epoch-1 `best.pt` 对完整 val 17,490 条生成回答，GPU 约 22.5/24.6 GB。最终 LoRA 严格指标尚未产生；同类别打乱 token 对照将在正确 token 的完整生成与评测结束后自动开始。

## 76. v3 最终评测与标签偏置诊断（2026-09-24）

1. 自动流水线于 00:23:41 写入 `ALL_DONE`。Projector、epoch-1 最佳 LoRA、同类别打乱 token 三组完整 val 评测均生成，共 17,490 条；日志未发现 OOM、NaN 或 Traceback。
2. LoRA 最终指标：关系关键词 42.07%、严格三元组 35.46%、完整标准句 64.13%、一句话 99.97%、规划动作 76.37%、A/B 交换一致性 30.00%、严格语言合取 63.09%、Grounding 正确条件 68.73%、端到端 30.09%。
3. 相对 Projector，LoRA 的关系关键词、严格三元组、完整标准句、规划动作、交换一致性、严格合取、Grounding 条件和端到端分别提高 9.01/7.55/6.59/2.25/5.49/6.92/5.80/2.54 个百分点。
4. 同类别打乱 token 的严格合取为 45.44%，LoRA 正确 token 高 17.65 个百分点；关系关键词、严格三元组、A/B 交换、Grounding 条件和端到端分别高 20.19/21.01/20.66/19.87/8.70 个百分点，说明模型确实利用了目标 token。
5. 分具体题型检查发现两处严重标签偏置：`ego_distance_band` 的正确/打乱 token 为 95.01%/95.22%，绝大多数目标超过 10 米；`candidate_maneuver_safety` 三组模型均为 87.11%，恰好等于 val 中 safe 标签比例 2,007/2,304。两项高分不能作为 token 空间推理证据。
6. 能证明 token 有效的题型及“正确 token − 打乱 token”增益为：ego 距离排序 +49.46 pp、两物体方向 +22.35 pp、局部避障规划 +14.06 pp、多参照方向 +13.46 pp、目标参照最近/最远 +11.66 pp。对应 LoRA 严格准确率为 92.15%/36.54%/34.90%/15.68%/60.88%。
7. 按大类严格合取为方位 32.60%、距离 83.58%、避障 74.06%；后两项被上述容易题型抬高。今后结果必须同时给具体题型，不能只报大类平均或总体 63.09%。
8. 按目标数 LoRA 为 N2/N3/N4/N5 = 70.80%/61.06%/43.67%/41.71%；相对打乱 token 的增益为 16.77/21.87/10.71/1.71 pp。N5 Grounding JointAll@0.25 为 0%，所以 41.71% 主要反映标签先验，不能解释为五目标端到端成功。
