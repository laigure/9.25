"""Select a retained Grounding checkpoint by validation multi-target score."""
...
def validation_metrics(log_text):
    ...
    for index, mark in enumerate(marks):
        ...
        groups = {int(n): float(score) for n, score in re.findall(
            r"bbf targets=(\d+): n=\d+\s+JointAcc@0\.25=([0-9.]+)", block)}
        if overall and all(count in groups for count in (2, 3, 4)):
            epoch = int(mark.group(1))
            results[epoch] = {
                "joint_all_bbf_025": float(overall.group(1)),
                "joint_by_target_count_025": groups,
                "macro_n2_n4_025": sum(groups[n] for n in (2, 3, 4)) / 3.0,
            }
    return results
...
def main():
    ...
    if not candidates:
        raise SystemExit("no retained checkpoint has matching validation metrics")
    epoch, checkpoint = max(
        candidates,
        key=lambda item: (metrics[item[0]]["macro_n2_n4_025"],
                          metrics[item[0]]["joint_all_bbf_025"]))
    result = {
        "selected_checkpoint": str(checkpoint),
        "selected_epoch": epoch,
        "selection_metric": "macro JointAll-bbf Acc@0.25 over N=2,3,4",
        "selected_metrics": metrics[epoch],
        "candidate_metrics": {
            str(candidate_epoch): metrics[candidate_epoch]
